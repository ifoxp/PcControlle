...
# UI/UX Expert Skill: Elite Python Desktop Design

## ⚠️ СУВОРІ ЗАБОРОНИ (HARD RESTRICTIONS)
- **Ніякого стандартного `tkinter`:** Категорично заборонено використовувати базові сірі віджети `tkinter`. Якщо проєкт на Tkinter, використовуй ТІЛЬКИ `customtkinter` (CTk) з правильними темами.
- **Ніяких емодзі та текстової псевдографіки:** Для стрілок, іконок та чекбоксів заборонено використовувати символи (▲, ▼, 🗸). 
- **Жодних стандартних рамок вікна:** Завжди приховуй системний заголовок вікна (наприклад, `overrideredirect(True)` для Tkinter або `Qt.FramelessWindowHint` для PyQt) і створюй кастомний TitleBar у стилі додатку.

## 🎨 ТРЕНДОВИЙ UI/UX ТА КОНТРАСТ
- **Стилістика Bento Grid:** Розбивай інтерфейс на ізольовані картки/фрейми з відступами.
- **Кольори (Dark Theme):** Використовуй палітру темних відтінків (фон `#1A1A1A`, картки `#2B2B2B`). Текст має бути висококонтрастним: `#FFFFFF` для заголовків, `#B3B3B3` для другорядного тексту. Ніколи не роби темно-синій текст на чорному фоні.
- **Кастомні контролі:** CheckBox та RadioButton мають бути чіткими та зрозумілими. Заборонено робити їх у вигляді суцільних зафарбованих квадратів. 

## 📐 СІТКА, ВІДСТУПИ ТА МАСШТАБУВАННЯ
- Уникай жорсткої фіксації розмірів (наприклад, `width=200, height=50`). Використовуй динамічне компонування (`pack(expand=True)`, `grid(sticky="nsew")` або `QVBoxLayout`/`QHBoxLayout`), щоб при зміні тексту або ресайзі вікна нічого не налазило одне на одне.
- Завжди додавай внутрішні відступи (`padx`, `pady` або `setContentsMargins`), мінімум 10-15px, щоб текст не прилипав до країв карток.
- Обов'язково вмикай підтримку High DPI (наприклад, `ctypes.windll.shcore.SetProcessDpiAwareness(1)` для Windows), щоб інтерфейс не був "мильним" на сучасних моніторах.

## 🖼️ ІКОНКИ ТА ГРАФІКА
- Для іконок всередині інтерфейсу використовуй лише векторну графіку (SVG або малювання через Canvas/QPainter) або спеціальні шрифти іконок (наприклад, `FontAwesome`). Заборонено вставляти `.png` або `.jpg` низької якості.
